...
作为 Demo 常规入口，负责展示 TripKin 的旅行探索入口，并承接页面内 MBTI 入口。
...
MBTI 是旅行人格生成和更新入口，不再长期占用底部导航。它应从 Home、Profile 的人格区域，以及必要的页面内卡片或 CTA 进入。需要解释上下文时，可以使用 `MbtiEntryModal` 做页面内引导；该弹窗不属于 BottomNav 职责。
