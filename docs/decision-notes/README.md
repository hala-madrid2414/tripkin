...
- 类型：工程化 / 协作规则
- 背景：并非所有文档整理都会影响页面运行。对纯 Markdown 说明、协作记录、需求整理或决策记录强制运行 `npm run lint` 和 `npm run build`，会增加不必要的本地耗时和噪音。
- 决定：仅修改纯文档内容，且不改变代码、样式、路由、脚本、依赖、工程配置、服务端代码或可执行命令时，不要求运行 `npm run lint` 和 `npm run build`。如果文档改动会影响工程执行方式或包含需要验证的命令，按实际影响补充对应验证。
- 影响范围：团队提交前检查、AI 协作收口说明、文档整理类任务。
- 后续注意：这个规则不降低代码改动的验证要求。凡是影响运行行为、构建链路或共享组件的改动，仍需按原规则运行 `npm run lint` 和 `npm run build`。

### 2026-06-21 调整底部主导航为首页 / 地图 / 匹配 / 我的

- 类型：公共组件 / 产品导航 / 协作规则
- 背景：Match 是 TripKin 的核心产品主链路之一，用户需要从底部导航随时进入搭子 / 行程匹配。MBTI 仍是重要能力，但它的职责是生成或更新旅行人格，不适合长期占用底部主导航。
- 决定：底部导航统一为 `首页 / 地图 / 匹配 / 我的`。`BottomNav` 是跨页面公共组件，Match 作为底栏核心入口；MBTI 改由 Home、Profile 的人格区域，以及必要页面内卡片或 CTA 进入。`MbtiEntryModal` 只服务明确的页面内 MBTI 引导，不再作为底部导航行为的一部分。
- 影响范围：`src/components/BottomNav`、Home、Map、Match、Profile、MBTI 入口说明、相关验收标准、页面协作文档和后续导航复验。
- 后续注意：2026-06-20 关于 `首页 / 地图 / MBTI / 我的` 的底栏决策已被本决策取代。后续页面负责人不要私自修改 `BottomNav` 一级入口；如再次调整一级入口，必须同步本决策记录、PRD、整改范围和相关验收文档。
...
- 后续注意：公共骨架只做到能挂载、能跳转、不会挡住页面负责人工作；不要在骨架维护任务里夹带完整首页运营内容或单页视觉重写。外部短视频入口、消息页、维度分析页、深度推荐算法、真实分享和原型图严格复刻仍未确认，不进入正式执行范围。本条中的 `首页 / 地图 / MBTI / 我的` 底栏方案已被 2026-06-21 `首页 / 地图 / 匹配 / 我的` 决策取代。
...
- 后续注意：该决策先被“2026-06-20 落地首页与全局 MBTI 入口公共骨架”更新，随后又被 2026-06-21 `首页 / 地图 / 匹配 / 我的` 决策取代。当前 `BottomNav` 以 `首页 / 地图 / 匹配 / 我的` 为准。不要为单个页面私自改公共导航结构；如新增或移除一级入口，需要同步更新本决策记录和相关页面验收。
