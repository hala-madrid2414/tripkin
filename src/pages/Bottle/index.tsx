...
function Bottle() {
  ...
  const baseBottles = useMemo(
    ...
    [bottleResult.items, currentDestination.id, userBottlesByDest],
  )
  ...
  const handleSendBottle = () => {
    ...
    const newBottle: BottleMessage = {
      ...
      spotId: currentDestination.id,
      regionId: currentDestination.parentId ?? currentDestination.id,
      title: `我在${currentDestination.name}投出的漂流瓶`,
      content,
      ...
      tags: [
        currentDestination.name,
        visibilityLabels[selectedBottleType],
        '刚投出',
      ],
      likes: 0,
      comments: 0,
